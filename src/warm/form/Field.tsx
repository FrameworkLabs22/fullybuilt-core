...
export function Field({ label, required, hint, error, className, children }: FieldProps) {
  ...
  return (
    <div className={cn("fb-field", className)}>
      <label htmlFor={id} className={cn("fb-label", required && "fb-label--req")}>
        {label}
      </label>
      {children({
        id,
      ...
      {note && (
        <span id={noteId} className={cn("fb-hint", error && "fb-hint--err")}>
          {note}
        </span>
      )}
    </div>
...
export function Label({
  ...
  return <label className={cn("fb-label", required && "fb-label--req", className)} {...props} />;
}
...
export function Hint({
  ...
  return <span className={cn("fb-hint", error && "fb-hint--err", className)} {...props} />;
}
